...
public final class Utility {
	...
	public static void loadMapAsset(String mapFilenamePath){
		...
		if(_filePathResolver.resolve(mapFilenamePath).exists()){
			...
			Gdx.app.debug(TAG, "Map doesn't exit" + mapFilenamePath);
		}
	}
	
	// retrieve the asset once loaded
	...
	public static void loadTextureAsset(String textureFilenamePath){
		...
		if(_filePathResolver.resolve(textureFilenamePath).exists()){
			...
			//Lock the thread until loaded
			_assetManager.finishLoadingAsset(textureFilenamePath);			
		}
		else{
			Gdx.app.debug(TAG, "Texture doesn't exits: " + textureFilenamePath);
		}
	}
