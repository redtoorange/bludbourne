...
public class MapManager {
	...
	private Hashtable<String, String> _mapTable;	// container for all the map locations on disk
	private Hashtable<String, Vector2> _playerStartLocationTable;	// container for all the start locations
	...
	public void loadMap(String mapName){
		...
		if(mapFullPath == null || mapFullPath.isEmpty()){
			Gdx.app.debug(TAG, "Map doesn't exist: " + mapName);
		}
	...
	// the player start would be in pixel coordinates, we need to convert it to world units
	...
	private void setClosestStartPosition(final Vector2 position){
		...
		_playerStartLocationTable.put(_currentMapName, _closestPlayerStartPosition);
	}
	
	public void setClosestStartPositionFromScaledUnit(Vector2 position){
		if(UNIT_SCALE <= 0){
			return;
		}
